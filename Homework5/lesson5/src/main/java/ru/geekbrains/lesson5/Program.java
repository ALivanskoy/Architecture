...
public class Program {
    ...
    public static void main(String[] args) {
        GazeboficationEditor3D editor3D = new GazeboficationEditor3D();
        boolean f = true;
        while (f){
            ...
            System.out.println("8. Превратить все модели в Газебо");
...
class Editor3D implements UILayer{
    ...
    protected BusinessLogicalLayer businessLogicalLayer;

    protected DatabaseAccess databaseAccess;

    protected GazeboficationDatabase database;


    protected void initialize(){
        database = new GazeboEditorDatabase(projectFile);
...
class GazeboficationEditor3D extends Editor3D implements UILayerGazebofication {
    @Override
    public void changeModelToGazebo() {
        database.useGazeboNow();
...
/**
 * Интерфейс, добавляющий новую фишку)
 */
interface UILayerGazebofication {

    void changeModelToGazebo();
}
...
class EditorDatabase implements Database{
    ...
    protected void generateModelAndTextures(){
...
/**
 * Расширение с возможностями использования новой сущности
 */
class GazeboEditorDatabase extends EditorDatabase implements GazeboficationDatabase {

    public GazeboEditorDatabase(ProjectFile projectFile) {
        super(projectFile);
    }

    @Override
    public void useGazeboNow() {

        for (Entity entry: super.entities) {
            if (entry instanceof Model3D){
                entry = null;
                entry = new Gazebo();
            }
        }
...
/**
 * 3D модель определённого назначения
 */
class Gazebo extends Model3D implements Entity{
    ...
    @Override
    public String toString() {
        return String.format("Awesome Gazebo #%s", id);
    }
...
/**
 * Интерфейс работы с новым видом 3D моделей
 */
interface GazeboficationDatabase extends Database {
